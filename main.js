...
const heroNameLabel = document.getElementById('hero-name');
const modalEl = document.getElementById('creation-modal');
const startBtn = document.getElementById('start-game');
const nameInput = document.getElementById('hero-name-input');
const pillGender = Array.from(document.querySelectorAll('.pill'));
...
function spawnEnemies() {
  ...
  document.getElementById('biome').textContent = `Биом: ${biome.name}`;
  ...
  for (let i = 0; i < visible; i++) {
    ...
    enemy.dataset.offset = String(240 + i * 30);
...
function animateHeroStrike() {
  heroEl.classList.add('attack');
  setTimeout(() => heroEl.classList.remove('attack'), 320);
}

function animateHeroHit() {
  if (heroEl.classList.contains('hit')) return;
  heroEl.classList.add('hit');
  setTimeout(() => heroEl.classList.remove('hit'), 360);
}
...
function moveEnemies() {
  ...
  enemies.forEach((enemy, idx) => {
    ...
    const stored = Number(enemy.dataset.offset ?? 220);
    const nextOffset = Math.max(0, stored - (6 + idx));
    enemy.dataset.offset = String(nextOffset);
    enemy.style.transform = `translateX(${-nextOffset}px)`;
    if (dist <= 60 && !enemy.classList.contains('leap')) {
      enemy.classList.add('leap');
      setTimeout(() => enemy.classList.remove('leap'), 580);
      animateHeroHit();
    }
    if (dist <= 0 && !enemy.classList.contains('jump')) {
      enemy.classList.add('jump');
      setTimeout(() => enemy.classList.remove('jump'), 520);
    }
  });
}
...
function setupInteractions() {
  ...
  document.getElementById('btn-comb').addEventListener('click', () => openModal(true));
}
...
function updateHeroLayers() {
  const current = customizationOptions[appearanceState.gender];
  slotPreviews.head.src = current.head[appearanceState.headIndex];
  slotPreviews.torso.src = current.torso[appearanceState.torsoIndex];
  slotPreviews.armor.src = current.armor[appearanceState.armorIndex];
  slotPreviews.weapon.src = customizationOptions.weapons[appearanceState.weaponIndex];

  heroEl.querySelector('.head').src = current.head[appearanceState.headIndex];
  heroEl.querySelector('.torso').src = current.torso[appearanceState.torsoIndex];
  heroEl.querySelector('.armor').src = current.armor[appearanceState.armorIndex];
  heroEl.querySelector('.weapon').src = customizationOptions.weapons[appearanceState.weaponIndex];
  heroNameLabel.textContent = appearanceState.name;
}

function cycleSlot(slot, direction) {
  const opts = slot === 'weapon' ? customizationOptions.weapons : customizationOptions[appearanceState.gender][slot];
  const key = `${slot}Index`;
  appearanceState[key] = (appearanceState[key] + direction + opts.length) % opts.length;
  updateHeroLayers();
}
...
function bindSlotButtons() {
  document.querySelectorAll('[data-slot-btn]').forEach((btn) => {
    btn.addEventListener('click', () => {
      const slot = btn.dataset.slotBtn;
      const dir = btn.classList.contains('slot-prev') ? -1 : 1;
      cycleSlot(slot, dir);
    });
  });
}

function bindGenderToggle() {
  pillGender.forEach((pill) => {
    pill.addEventListener('click', () => {
      pillGender.forEach((p) => p.classList.remove('active'));
      pill.classList.add('active');
      appearanceState.gender = pill.dataset.gender;
      appearanceState.headIndex = 0;
      appearanceState.torsoIndex = 0;
      appearanceState.armorIndex = 0;
      updateHeroLayers();
    });
  });
}

function openModal(fromComb = false) {
...
function setupModal() {
  bindSlotButtons();
  bindGenderToggle();
  nameInput.value = appearanceState.name;
  startBtn.addEventListener('click', () => {
    const value = nameInput.value.trim();
    appearanceState.name = value || 'Герой';
    heroNameLabel.textContent = appearanceState.name;
    updateHeroLayers();
    closeModal();
    startGameFlow();
