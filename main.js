...
const creatorHero = document.getElementById('creator-hero');
...
const creatorConfirm = document.getElementById('creator-confirm');
...
const state = {
  ...
  gender: 'male',
  appearance: {
    head: customization.male.head[0],
    torso: customization.male.torso[0],
    armor: customization.male.armor[0],
    weapon: customization.weapons[0]
  },
...
function spawnEnemies() {
  ...
  for (let i = 0; i < visible; i++) {
    ...
    enemy.style.setProperty('--offset', `${-i * 24}px`);
  ...
  setBattleBackground(biome.folder);
}
...
function moveEnemies() {
  ...
  enemies.forEach((enemy, idx) => {
    ...
    const offset = Math.max(-320, dist - 140 - idx * 14);
    const hop = dist < 160 ? 'pounce' : '';
    enemy.style.setProperty('--offset', `${offset}px`);
    enemy.classList.toggle('pounce', hop === 'pounce');
    if (dist <= 60 && !enemy.dataset.hit) {
      enemy.dataset.hit = '1';
...
function applyAppearance(target, appearance) {
  target.querySelector('.layer.head').src = appearance.head;
  target.querySelector('.layer.torso').src = appearance.torso;
  target.querySelector('.layer.armor').src = appearance.armor;
  target.querySelector('.layer.weapon').src = appearance.weapon;
}

function randomizeAppearance() {
  const pool = customization[state.gender];
  state.appearance = {
    head: pool.head[Math.floor(Math.random() * pool.head.length)],
    torso: pool.torso[Math.floor(Math.random() * pool.torso.length)],
    armor: pool.armor[Math.floor(Math.random() * pool.armor.length)],
    weapon: customization.weapons[Math.floor(Math.random() * customization.weapons.length)]
  };
  applyAppearance(creatorHero, state.appearance);
}

function bindCreator() {
  populateSelect(headSelect, customization[state.gender].head);
  populateSelect(torsoSelect, customization[state.gender].torso);
  populateSelect(armorSelect, customization[state.gender].armor);
  populateSelect(weaponSelect, customization.weapons);
  headSelect.value = state.appearance.head;
  torsoSelect.value = state.appearance.torso;
  armorSelect.value = state.appearance.armor;
  weaponSelect.value = state.appearance.weapon;
  applyAppearance(creatorHero, state.appearance);
}
...
function setBattleBackground(folder) {
  const back = document.querySelector('.battle .layer-back');
  const mid = document.querySelector('.battle .layer-mid');
  const biomeBg1 = {
    '1 уровень Злой лес': 'sprites/Фоны/Фон прохождения дня 1.png',
    '2 уровень Лиходеи': 'sprites/Фоны/Фон прохождения дня 2.png',
    '3 уровень Бусурмане': 'sprites/Фоны/Фон прохождения дня 1.png',
    '4 уровень Иноземцы': 'sprites/Фоны/Фон прохождения дня 2.png',
    '5 уровень Мертвяки': 'sprites/Фоны/Фон прохождения дня 1.png',
    '6 уровень Нечисть': 'sprites/Фоны/Фон прохождения дня 2.png',
    '7 уровень Черти': 'sprites/Фоны/Фон прохождения дня 1.png',
    '8-10 уровни Случайные боссы': 'sprites/Фоны/Фон прохождения дня 2.png'
  };
  const biomeBg2 = {
    '1 уровень Злой лес': 'sprites/Фоны/Фон прохождения дня 2.png',
    '2 уровень Лиходеи': 'sprites/Фоны/Фон прохождения дня 1.png',
    '3 уровень Бусурмане': 'sprites/Фоны/Фон прохождения дня 2.png',
    '4 уровень Иноземцы': 'sprites/Фоны/Фон прохождения дня 1.png',
    '5 уровень Мертвяки': 'sprites/Фоны/Фон прохождения дня 2.png',
    '6 уровень Нечисть': 'sprites/Фоны/Фон прохождения дня 1.png',
    '7 уровень Черти': 'sprites/Фоны/Фон прохождения дня 2.png',
    '8-10 уровни Случайные боссы': 'sprites/Фоны/Фон прохождения дня 1.png'
  };
  back.style.backgroundImage = `url(${biomeBg1[folder]})`;
  mid.style.backgroundImage = `url(${biomeBg2[folder]})`;
}
...
function setupInteractions() {
  ...
  [headSelect, torsoSelect, armorSelect, weaponSelect].forEach((select) => {
    select.addEventListener('change', () => {
      state.appearance = {
        head: headSelect.value,
        torso: torsoSelect.value,
  ...
  creatorCancel.addEventListener('click', () => {
    randomizeAppearance();
    bindCreator();
  });
  creatorConfirm.addEventListener('click', () => {
    state.heroName = nameInput.value.trim() || 'Иван';
    applyAppearance(heroEl, state.appearance);
    hideCreator();
  });
}
